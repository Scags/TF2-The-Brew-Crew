...
#define SOUND_MELEE			"vo/heavy_meleedare02.mp3"
#define SOUND_BLIP			"buttons/blip1.wav"
#define SOUND_NOSTALGIA		"ui/tv_tune.mp3"
...
enum struct g_ePerks
{
	char g_strPerkName[STRING_PERK_MAXLEN];
	char g_strPerkDesc[STRING_PERK_MAXLEN];
	char g_strPerkKey[STRING_PERK_MAXLEN];
	g_ePerkType g_nPerkType;
	bool g_bPerkDisabled;
	float g_flPerkTime;
}
g_ePerks g_nPerks[MAX_RTD_EFFECTS];

enum g_eCurrentPerk
{
	PERK_GODMODE=0,
	PERK_TOXIC = 1,
	PERK_BUFFED_HEALTH = 2,
	PERK_SPEED = 3,
	PERK_NOCLIP = 4,
	PERK_LOW_GRAVITY = 5,
	PERK_UBER = 6,
	...
	PERK_CLOAK = 8,
	PERK_CRITS = 9,
	PERK_INFINITE_AMMO = 10,
	PERK_SCARY_BULLETS = 11,
	PERK_SENTRY = 12,
	PERK_HOMING = 13,
	PERK_CHARGE = 14,
	PERK_EXPLODE = 15,
	...
	PERK_NOSTALGIA = 26,
	PERK_EARTHQUAKE = 27,
	PERK_FUNNY_FEELING = 28,
	PERK_SAUCE = 29,
	PERK_DISPENSER = 30,
	PERK_JUMP = 31,
	PERK_INSTANT_KILLS = 32,
	PERK_BIG_HEAD = 33,
	PERK_TINY_PLAYER = 34
};
...
g_ePlayerInfo g_nPlayerData[MAXPLAYERS+1];
...
new g_nWeaponCache[MAXPLAYERS+1][3];
...
public OnPluginStart()
{
	...
	g_hCvarVersion = CreateConVar("sm_rtd_version", PLUGIN_VERSION, "Current RTD Version", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
...
public APLRes:AskPluginLoad2(Handle:myself, bool:late, String:error[], err_max)
{
	...
	CreateNative("RTD_GetRollType", Native_GetRollType);
...
bool:CanBeRolled(client, g_eCurrentPerk:nPerk)
{
	...
	switch(nPerk)
	{
		...
		case PERK_CLOAK:
		{
			if(class != TFClass_Spy) return false;
			if (GetClientCloakIndex(client) == 60) return false;
		}
		...
		case PERK_JUMP: if(class != TFClass_Scout && !VSH2Player(client).bIsBoss) return false;
		case PERK_INFINITE_AMMO:
		{
			int wep = GetPlayerWeaponSlot(client, TFWeaponSlot_Primary);
			if (wep > MaxClients && GetEntProp(wep, Prop_Send, "m_iItemDefinitionIndex") == 730)
				return false;
		}
	}
...
InitiateEffect(client, g_eCurrentPerk:nPerk)
{
	...
	switch(nPerk)
	{
		...
		case PERK_BUFFED_HEALTH:
		{
			...
			int iHealth = GetConVarInt(g_hCvarHealth);
			if (player.bIsBoss)
			{
				player.iHealth += GetConVarInt(g_hCvarHealth);
				if (player.iHealth > player.iMaxHealth)
					player.iHealth = player.iMaxHealth;

				iHealth = player.iHealth;
			}
			else
			{
		...
		case PERK_JUMP:
		{
			PrintToChatAll("%s %T", PLUGIN_PREFIX, "RTD_Effect", LANG_SERVER, g_strTeamColors[iTeam], client, 0x01, g_nPerks[_:nPerk].g_nPerkType == PERK_GOOD ? COLOR_PERK_GOOD : COLOR_PERK_BAD, (VSH2Player(client).bIsBoss ? "Greater Superjumps" : g_nPerks[_:nPerk].g_strPerkName), 0x01);
...
public Action:Timer_CheckStuck(Handle:hTimer, any:iUserId)
{
	...
	{
		PrintToChat(client, "%s %T", PLUGIN_PREFIX, "RTD_Stuck", LANG_SERVER);
		if (!TeleportToSpawn(client, GetClientTeam(client)))
...
public Action:Event_PlayerDeath(Handle:hEvent, const String:strEventName[], bool:bDontBroadcast)
{
	...
	if (VSH2Player(iVictim).GetProperty("iHealth") > 0) return Plugin_Continue;
...
stock void SetGodmode(client, bool:bEnabled)
{
	new iGodmode = GetConVarInt(g_hCvarBuddah) ? 1 : 0;
	SetEntProp(client, Prop_Data, "m_takedamage", bEnabled ? iGodmode : 2, 1);
}
...
public any Native_GetRollType(Handle plugin, int numParams)
{
	int client = GetNativeCell(1);
	if (!(0 < client <= MaxClients) || !IsClientInGame(client) || !IsPlayerAlive(client))
		return -1;
	return g_nPlayerData[client].g_nPlayerState == STATE_ROLLING ? g_nPlayerData[client].g_nPlayerPerk : g_eCurrentPerk:-1;
}
...
{
	SetEntData(client, g_iOffsetAmmo+(4*3), iMetal);  
}
...
stock bool TeleportToSpawn(const int client, int team = 0)	// Props to Chdata!
{
	int iEnt = -1;
	float vPos[3], vAng[3];
